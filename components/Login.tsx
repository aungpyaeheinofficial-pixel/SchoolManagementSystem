...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const LoginForm = ({ hideMobileLogo = false }: { hideMobileLogo?: boolean }) => (
    <div className="w-full max-w-md relative z-10">
      {/* Mobile Logo (optionally hidden when shown inside modal) */}
      {!hideMobileLogo && (
  ...
  return (
    <div className="min-h-screen flex">
      {/* Mobile: Cover animation page + login popup (ONLY on mobile) */}
      <div className="md:hidden w-full min-h-screen bg-gradient-to-br from-brand-600 to-brand-700 relative overflow-hidden flex items-center justify-center px-6">
        {/* Animated background */}
        <div className="absolute inset-0">
          <div className="absolute -top-40 -left-40 w-96 h-96 bg-white/10 rounded-full blur-3xl pn-anim-float-2 pn-anim-shimmer"></div>
          <div className="absolute top-1/2 -right-24 w-96 h-96 bg-white/10 rounded-full blur-3xl pn-anim-float-1 pn-anim-shimmer" style={{ animationDelay: '0.8s' }}></div>
          <div className="absolute -bottom-32 left-1/3 w-80 h-80 bg-white/10 rounded-full blur-3xl pn-anim-float-2 pn-anim-shimmer" style={{ animationDelay: '1.6s' }}></div>
          ...
          {/* Extra moving sparkles (very visible) */}
          <div className="absolute top-24 right-10 w-3 h-3 bg-white/40 rounded-full pn-anim-float-1"></div>
          <div className="absolute top-40 left-10 w-2.5 h-2.5 bg-white/35 rounded-full pn-anim-float-2"></div>
          <div className="absolute bottom-28 right-24 w-4 h-4 bg-white/25 rounded-full pn-anim-float-1"></div>
        </div>
        ...
        <div className="relative z-10 w-full max-w-sm text-center">
          ...
          <div className="mt-6 px-2">
            ...
            <p className="text-white/80 font-burmese mt-2 leading-relaxed">
              ပညာရေးကို နည်းပညာဖြင့် အားဖြည့်ပေးခြင်း
            </p>
          </div>

          {/* Stats (must be visible on mobile cover) */}
          <div className="mt-6 grid grid-cols-3 gap-3">
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-md py-3">
              <p className="text-2xl font-extrabold text-white">500+</p>
              <p className="text-xs font-bold text-white/70 mt-1">Students</p>
            </div>
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-md py-3">
              <p className="text-2xl font-extrabold text-white">50+</p>
              <p className="text-xs font-bold text-white/70 mt-1">Teachers</p>
            </div>
            <div className="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-md py-3">
              <p className="text-2xl font-extrabold text-white">15+</p>
              <p className="text-xs font-bold text-white/70 mt-1">Years</p>
            </div>
          </div>

          {/* Floating Feature Cards (obvious motion) */}
          <div className="mt-8 flex justify-center gap-4">
            {floatingItems.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`w-24 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md px-3 py-3 transition-all duration-700 ${
                    isActive ? 'scale-110 -translate-y-2 shadow-2xl' : 'opacity-80'
                  }`}
                  style={{ transitionDelay: item.delay }}
                >
                  <div className={`mx-auto mb-2 w-10 h-10 rounded-xl flex items-center justify-center ${
                    isActive ? 'bg-white text-brand-700' : 'bg-white/15 text-white'
                  }`}>
                    <Icon size={20} />
                  </div>
                  <p className="text-xs font-extrabold text-white">{item.text}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="mt-10">
            <button
              ...
              className="w-full py-4 rounded-2xl bg-white text-brand-700 font-extrabold text-lg shadow-2xl transition-all hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2"
            >
              Login
              <ArrowRight size={20} />
            </button>
            <p className="mt-4 text-sm text-white/70">
              Tap to sign in and continue
            </p>
          </div>
        </div>

        {/* Mobile Popup Form (bottom sheet) */}
        {showMobileForm && (
          <div
          ...
          >
            <div className="w-full max-w-md bg-gradient-to-br from-slate-50 to-slate-100 rounded-[32px] shadow-2xl max-h-[90vh] overflow-hidden pn-modal-panel">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200/70">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-extrabold text-slate-900">Sign in</p>
                    <p className="text-xs text-slate-500 font-burmese">အကောင့်ဝင်ရန်</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setShowMobileForm(false)}
                  className="p-2 bg-white/70 hover:bg-white rounded-xl text-slate-500 pn-modal-close"
                  aria-label="Close"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="pn-modal-body pn-modal-compact p-5">
                {/* Hide the duplicate mobile logo inside the popup */}
                <LoginForm hideMobileLogo />
              </div>
            </div>
          </div>
        )}
      ...
      {/* Show decorative panel on md+; hide on small screens to prevent layout issues */}
      <div className={`hidden md:flex md:w-[48%] xl:w-[55%] bg-gradient-to-br ${currentRole.color} relative overflow-hidden`}>
      ...
      <div className="hidden md:flex md:w-[52%] xl:w-[45%] items-center justify-center p-6 sm:p-12 bg-gradient-to-br from-slate-50 to-slate-100 relative overflow-hidden">
        <LoginForm />
      </div>
    </div>
  );
